...
export const mentionsUsersSuggestionOptions: MentionOptions["suggestion"] = {
  char: "@",
  ...
  items: async ({ query }): Promise<MentionSuggestion[]> => {
    ...
    const debouncedCall = AwesomeDebouncePromise(
      (query: string) =>
        api.get<MentionSuggestion[]>(
          `mention?page=0&page_size=20&query=${query}`,
        ),
      200,
    );
...
const MentionList = forwardRef<MentionRef, MentionProps>((props, ref) => {
  ...
  const selectItem = (index: number) => {
    ...
    const mentionItem = {
      ...
      avatar: suggestion.profile_picture,
      ...
      {props.items.map((item, index) => (
        ...
        >
          <div className="flex items-center gap-2">
            <UserAvatar
              fallbackText={item.profile_picture.charAt(0)}
              userId={item.id}
            />
            {item.user_name}
            {item.verified && (
              <CheckBadgeIcon className="size-5 rounded-full text-blue-400" />
            )}
          </div>
        </button>
      ))}
